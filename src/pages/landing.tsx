...
export function LandingPage() {
  ...
  // Google interactive states
  const [showGooglePicker, setShowGooglePicker] = useState(false)
  const [googleEmailInput, setGoogleEmailInput] = useState("")
  const [googleNameInput, setGoogleNameInput] = useState("")
  const [googleAuthLoading, setGoogleAuthLoading] = useState(false)
  const [googleLoadingText, setGoogleLoadingText] = useState("")
  const [googlePickerMode, setGooglePickerMode] = useState<"select" | "custom">("select")
  ...
  const handleGoogleSignIn = useCallback(() => {
    ...
    setGooglePickerMode("select")
    setGoogleEmailInput("")
    setGoogleNameInput("")
    setShowGooglePicker(true)
  }, [])

  const triggerGoogleLogin = useCallback((email: string, fullName: string) => {
    setGoogleAuthLoading(true)
    setGoogleLoadingText("Connecting to accounts.google.com...")
    
    setTimeout(() => {
      setGoogleLoadingText("Verifying credentials & security handshake...")
      setTimeout(() => {
        setGoogleLoadingText("Authorizing DigiHealth platform access...")
        setTimeout(() => {
          const res = loginWithIdentifier(email, fullName)
          setGoogleAuthLoading(false)
          setShowGooglePicker(false)
          if (res.success) {
            toast.success(`✅ Authenticated via Google!`)
          } else if (res.isNew) {
            toast.success(`🎉 Google Account Connected! Please complete your onboarding.`)
          }
        }, 600)
      }, 600)
    }, 600)
    ...
    <div ref={mainRef} style={{ background: "#030712", color: "#f8fafc", fontFamily: "'Inter', system-ui, sans-serif", overflowX: "hidden" }}>
      ...
      {showGooglePicker && (
        <div
          ...
          onClick={e => !googleAuthLoading && e.target === e.currentTarget && setShowGooglePicker(false)}
        >
          <div
            ...
            style={{
              ...
              boxShadow: "0 24px 64px rgba(0, 0, 0, 0.6), 0 0 50px rgba(6, 182, 212, 0.08)",
              animation: "modalIn 0.3s cubic-bezier(0.34,1.56,0.64,1)",
              padding: "28px",
            }}
          >
            ...
            <p className="text-xs text-slate-400 text-center mb-6">
              to continue to <span className="text-cyan-400 font-semibold">DigiHealth</span>
            </p>
            ...
            ) : googlePickerMode === "select" ? (
              <div className="space-y-3">
                {/* Account list */}
                {[
                  { name: "Abhishek Panda", email: "abhishek.panda@gmail.com", desc: "Creator & Owner" },
                  { name: "Dr. Priya Sharma", email: "dr.priyasharma@gmail.com", desc: "Cardiologist, AIIMS" },
                  { name: "Guest Explorer", email: "guest.health@gmail.com", desc: "Individual Sandbox" }
                ].map(acc => (
                  <button
                    key={acc.email}
                    onClick={() => triggerGoogleLogin(acc.email, acc.name)}
                    className="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-white/5 border border-white/5 hover:border-cyan-500/30 transition-all text-left group"
                  >
                    <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-cyan-500 to-indigo-500 flex items-center justify-center font-bold text-white text-sm shadow-md">
                      {acc.name.split(" ").map(n => n[0]).join("")}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-bold text-slate-200 group-hover:text-white transition-colors">{acc.name}</div>
                      <div className="text-xs text-slate-400 truncate">{acc.email}</div>
                    </div>
                    <div className="text-[10px] bg-white/5 border border-white/10 px-2 py-0.5 rounded text-slate-400 font-mono">
                      {acc.desc}
                    </div>
                  </button>
                ))}
                ...
                <div className="flex items-center gap-3 py-2">
                  ...
                  <span className="text-[10px] uppercase font-semibold text-slate-500 tracking-wider">or</span>
                  <div className="flex-1 h-px bg-white/5" />
                </div>

                {/* Use another account button */}
                <button
                  onClick={() => setGooglePickerMode("custom")}
                  className="w-full py-3 rounded-xl border border-white/10 hover:bg-white/5 text-sm font-semibold text-slate-300 hover:text-white transition-all text-center"
                >
                ...
                <form
                  onSubmit={e => {
                    ...
                    if (googleEmailInput.trim()) {
                      triggerGoogleLogin(googleEmailInput.trim().toLowerCase(), googleNameInput.trim() || "Google User")
                    }
                  }}
                  className="space-y-4"
                >
                  <div>
                    <label className="text-xs text-slate-400 mb-1 block">Google Email</label>
                    <input
                      type="email"
                      required
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g. Abhishek Panda"
                      className="w-full px-4 py-3 rounded-xl text-sm text-white placeholder-slate-500 outline-none"
                      style={{ background: "rgba(30,41,59,0.8)", border: "1px solid rgba(6,182,212,0.2)", boxSizing: "border-box" }}
                      onFocus={e => (e.target.style.borderColor = "rgba(6,182,212,0.6)")}
                      onBlur={e => (e.target.style.borderColor = "rgba(6,182,212,0.2)")}
                  ...
                  <div className="flex gap-3 pt-2">
                    <button
                      type="button"
                      onClick={() => setGooglePickerMode("select")}
                      className="flex-1 py-3 rounded-xl border border-white/10 hover:bg-white/5 text-sm font-semibold text-slate-400 hover:text-white transition-all"
                    >
                      ← Back
                    </button>
                    <button
