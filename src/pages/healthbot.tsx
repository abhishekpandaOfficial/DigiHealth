  ...
  useEffect(() => {
    setMessages([{
      ...
      content: activeMember
        ? `Hello! I'm DigiHealth AI, your health assistant. I have access to ${activeMember.name}'s health records. You can ask me about medications, vaccinations, lab results, medical history, or any health-related questions. You can also talk to me using the microphone button below!\n\n**Disclaimer:** I provide health information for educational purposes only. Always consult a qualified healthcare professional for medical advice, diagnosis, or treatment.`
...
`.trim()
  ...
  async function sendMessage(text?: string) {
    ...
    try {
      ...
      const systemPrompt = `You are DigiHealth AI, a helpful and empathetic digital health assistant for a family health management platform, developed by Abhishek Panda for DigiHealth Systems. You have access to a family member's complete health records provided in the context below.
    ...
    <div className="flex flex-col h-[calc(100vh-8rem)]">
      <div className="mb-4">
        <h1 className="text-2xl font-bold flex items-center gap-2">
          <Bot className="size-6 text-primary animate-bounce" /> DigiHealth AI
        </h1>
      ...
      <Card className="flex flex-col flex-1 overflow-hidden border-2 shadow-lg">
          ...
          {(loading || contextLoading) && (
            <div className="flex gap-3">
              ...
              <div className="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-2xl rounded-tl-sm px-4 py-3 flex items-center gap-2 shadow-sm">
                ...
                <span className="text-sm text-muted-foreground">{contextLoading ? "Loading health records..." : "DigiHealth AI is thinking..."}</span>
              </div>
            </div>
          )}
          <div ref={bottomRef} />
        ...
        <div className="px-4 py-2 flex items-center gap-2 text-xs text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/20 border-b border-amber-100/50 dark:border-amber-900/20">
          ...
          <span>DigiHealth AI provides info for reference only. Always seek professional advice for medical concerns.</span>
        </div>
        ...
        <div className="p-4 flex gap-2 bg-white dark:bg-slate-900">
          <Button 
            variant="ghost" 
            size="icon" 
            onClick={() => setMessages([{ role: "assistant", content: "Chat cleared. I'm DigiHealth AI, how can I help you today?", timestamp: new Date() }])}
            title="Clear Chat"
          >
          ...
          <Button 
            ...
            title={isMuted ? "Unmute DigiHealth AI Voice" : "Mute DigiHealth AI Voice"}
          ...
          <Input
            ...
            placeholder={activeMember ? `Ask DigiHealth AI about ${activeMember.name}'s health...` : "Select a member first..."}
            disabled={loading}
